...
  // ✅ NEW: Fix Missing Calorie Target
  if (!("dailyCalorieTarget" in state.profile) || state.profile.dailyCalorieTarget === undefined) {
    // Calculate dailyCalorieTarget from legacy dailyKcalTarget if possible, otherwise use 0 (for TDEE based target)
...
    state.profile.dailyCalorieTarget = state.profile.dailyKcalTarget !== undefined 
      ? state.profile.dailyKcalTarget - defaultTDEE // Calculate deficit from old dailyKcalTarget
      : 0; // Default to TDEE (0 deficit/surplus)
    
    // Remove legacy key to keep state clean (it's recalculated in handleSubmit for legacy app versions)
...
  const previewTDEE = Math.round(currentBmr * currentAf);
    // ✅ NEW: Daily Target Preview
  const previewDailyTarget = previewTDEE + (Number(form.dailyCalorieTarget) || 0);
...
    const toNumberOrEmpty = (v) => v === "" ? "" : Number.isNaN(Number(v)) ? "" : Number(v);
    // ✅ NEW: Helper for calorie target
    const calorieTarget = Number.isNaN(Number(form.dailyCalorieTarget)) ? 0 : Number(form.dailyCalorieTarget);
    const calculatedTDEE = Math.round(Number(form.bmr || 0) * (Number(form.defaultActivityFactor) || 1.2));
...
      // Legacy support - save the old field for apps not updated yet
...
          </div>
          ...
          {/* ✅ NEW: Calorie Target Input */}
          <div className="form-grid-1">
            <div className="form-group">
              <label htmlFor="dailyCalorieTarget">Calorie Deficit / Surplus Target (kcal)</label>
              <input
                id="dailyCalorieTarget" name="dailyCalorieTarget" type="number" step="10" placeholder="e.g. -500 for deficit, +300 for surplus, 0 for maintenance"
                value={form.dailyCalorieTarget} onChange={handleChange}
              />
              <p className="muted-text">A value like <strong>-500</strong> sets your goal to TDEE minus 500 kcal.</p>
            </div>
          </div>
...
              <div>
                  <span className="tdee-label">Estimated TDEE (Maintenance)</span>
...
          </div>
          {/* ✅ NEW: Daily Target Preview */}
          <div className="tdee-preview-box tdee-preview-total">
              <div>
                  <span className="tdee-label">Total Daily Calorie Goal</span>
                  <span className="tdee-sub">TDEE + Target Surplus/Deficit</span>
              </div>
              <div className="tdee-value">
                  {previewDailyTarget} kcal
              </div>
          </div>
